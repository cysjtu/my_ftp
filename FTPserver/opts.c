 ...
 */
int assign_option(int current_option, const char * arg , struct cmd_opts *copts, int len)
{
	int p;
	switch(current_option) { /*current_option 为命令的第一个字符，方便用来判断是哪个命令*/
		case 's': // server address   指定绑定到地址，最好是 ADDR_ANY
			...
			break;
		case 'p': // server port   监听哪个端口 默认为 21 端口
			p = toint(arg,TRUE);
			if (p<1 || p > 32768) {
				return 2;
			}
			copts->port = p;
		break;
		case 'l': // limit of the concurrent connections  允许的最大连接数
			p = toint(arg,TRUE);
			if (p<1 || p > 600) {
			...
			copts->max_conn = p;
		break;
		case 'u': // run as userid  以特定到 UID 运行，默认为ROOT 等下会调用函数，设定 RUID EUID
			p = toint(arg,TRUE);
			if (p<0 || p > 99999) {
			...
			copts->userid = p;
		break;
		case 'c': // current working directory 设定工作目录
			if(dir_exists(arg))
				return 5;
